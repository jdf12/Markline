...
(function () {
  ...
  // 翻译图标（地球+文字）
  const SVG_MDI_TRANSLATE = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 5h7"/><path d="M9 3v2c0 4.418-2.239 8-5 8"/><path d="M5 9c0 2.144 2.952 3.908 6.7 4"/><path d="M12 20l4-9 4 9"/><path d="M19.1 18h-6.2"/></svg>';
  ...
  class MDIWindowManager {
    ...
    _createWindowDOM(windowData) {
      ...
      win.innerHTML = `
        <div class="mdi-window-titlebar">
          ...
          <div class="mdi-window-controls">
            <button class="mdi-btn mdi-btn--translate" title="${escapeHtml(i18n('mdiTranslate'))}">${SVG_MDI_TRANSLATE}</button>
      ...
      // 翻译按钮：调用 TranslateOverlay 在此窗口上展开翻译工具栏
      const translateBtn = win.querySelector('.mdi-btn--translate');
      if (translateBtn) {
        translateBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          if (window.TranslateOverlay && typeof window.TranslateOverlay.toggle === 'function') {
            window.TranslateOverlay.toggle(id, url, win);
          }
        });
      }
