...
// ===== Prompt 构建 =====
function buildClassificationPrompt(bookmark, candidateTags, tagDescriptions) {
  ...
  return `你是一个书签分类助手。请根据书签的标题、URL、域名和描述，从下面的候选标签中选出最匹配的 1-3 个标签。
...
async function callAI(bookmark, candidateTags, tagDescriptions) {
  ...
  const prompt = buildClassificationPrompt(bookmark, candidateTags, tagDescriptions);
...
async function testAIConnection(config) {
  ...
  const tagDescriptions = typeof TAG_PROTOTYPES !== 'undefined' ? TAG_PROTOTYPES : {};
  const prompt = buildClassificationPrompt(testBookmark, [], tagDescriptions);
  const body = resolved.buildBody(prompt, resolved.model);
  const timeoutMs = Math.max(3000, (config.timeout || 8) * 1000);
